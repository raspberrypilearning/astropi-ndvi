...
The image that you have saved as `ndvi_contrasted` is not suitable for colour mapping. The numbers stored in the `numpy` array are currently all `floats` or what is commonly known as decimal numbers. They all need converting to whole numbers, or `integers` between `0` and `255`. Luckily, the `numpy` library can do this for us.
